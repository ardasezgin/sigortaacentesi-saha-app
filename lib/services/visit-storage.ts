...
export async function getDashboardMetrics(): Promise<DashboardMetrics> {
  try {
    ...
    const activeAgencies = agencies.filter(a => a.durum === 'Aktif').length;
    const passiveAgencies = agencies.filter(a => a.durum === 'Pasif').length;

    const totalVisitsThisWeek = visits.filter(
      v => new Date(v.visitDate) >= weekAgo
    ).length;

    const totalVisitsThisMonth = visits.filter(
      v => new Date(v.visitDate) >= monthAgo
    ).length;
